...
App.Pages.Admin = {
  ...
  async dashboard() {
    ...
    let stats = {};
    try { stats = await App.API.getStats(); stats = stats || {}; } catch(e) {}
    return `
  ...
  async manage(type) {
    ...
    try {
      let r;
      if (type === 'users') { r = await App.API.getUsers(); list = (r && r.rows) || []; }
      else if (type === 'orders') { r = await App.API.getOrders(); list = (r && r.rows) || []; }
      else if (type === 'messages') { r = await App.API.getMessages(); list = (r && r.rows) || []; }
    } catch(e) {}
  ...
  async editUser(id) {
    let r;
    try { r = await App.API.getUsers(); } catch(e) {}
  ...
  async editOrder(id) {
    let r;
    try { r = await App.API.getOrders(); } catch(e) {}
  ...
  async replyMsg(id) {
    let r;
    try { r = await App.API.getMessages(); } catch(e) {}
    const msgs = (r && r.rows) || [];
